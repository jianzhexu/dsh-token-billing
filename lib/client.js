...
`;
		...
		function BillingDockRow(props) {
			...
			const workspaces = useWorkspaces ? useWorkspaces((s) => (s ? s.items : undefined)) : undefined;
			...
			const g = state.global;
			// 归属工作区优先取客户端工作区表（当前会话所在的工作区实体，
			// 切换工作区即跟随，未记账的新会话同样正确）；回退 host 按记账
			// cwd 归属的 sessionWorkspace；两者皆无时回退全局汇总。
			let ws = null;
			if (hasSession && workspaces) {
				const owner = workspaces.find((w) => w && w.sessionIds && w.sessionIds.indexOf(sessionId) >= 0);
				if (owner) ws = state.workspaces.find((x) => x && String(x.id) === String(owner.workspaceId)) || null;
			}
			if (!ws) ws = state.sessionWorkspace;
			const groups = [];
			if (hasSession) {
				groups.push(["本会话 ", React.createElement("span", { key: "c0", className: "cost" }, `¥${fmtCost(state.totals.cost)}`)]);
				groups.push([`${ws ? ws.title : "全部会话"} `, React.createElement("span", { key: "c1", className: "cost" }, `¥${fmtCost(ws ? ws.totals.cost : g.totals.cost)}`)]);
			} else {
		...
		function apply(ctx) {
			...
			ctx.slots.inject("conversation.composer.dock", () => ctx.slots.register(
				...
				(props) => React.createElement(BillingDockRow, { useSessions: props.useSessions, useWorkspaces: props.useWorkspaces }),
			));
		}
