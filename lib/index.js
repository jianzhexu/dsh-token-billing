...
export default {
  ...
  async apply(ctx) {
    ...
    // 工作区注册表是可选依赖：apply 期一次性捕获会与服务的异步激活竞速
    // （拿到 undefined 后永不再查），改为使用时实时查找。
    const workspaceRegistry = () => ctx.get('workspaceRegistry')
    // 目录 -> workspace 实体索引（path -> { id, title }）
    const workspaceIndexOf = () => {
      const idx = new Map()
      const registry = workspaceRegistry()
      if (!registry) return idx
      let list = []
      try { list = registry.list() || [] } catch (e) { /* 忽略 */ }
      for (const w of list) {
    ...
    async function buildState(sessionId) {
      ...
      const registry = workspaceRegistry()
      const archived = new Set(registry ? (registry.archivedSessionIds || []) : [])
